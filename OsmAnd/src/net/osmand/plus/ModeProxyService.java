	...
	public IBinder onBind(Intent intent) {
		return new IModeService.Stub() {
			...
			public List<ModeData> getModes() throws RemoteException {
				...
				ModeData data = new ModeData(Modes.FULL_FEATURES, "Full features");
				modes.add(data);
				data = new ModeData(Modes.LIGHT_SCREEN, "Light screen");
				modes.add(data);
				data = new ModeData(Modes.DARK_SCREEN, "Dark screen");
				modes.add(data);
				data = new ModeData(Modes.AUDIO_ONLY, "Audio only");
				modes.add(data);
				return modes;
			}
		};
